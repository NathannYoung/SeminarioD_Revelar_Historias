...
window.AppDesk = {
  ...
  resetExperienceToStart: function() {
    ...
    if (droppedPhotoStage) droppedPhotoStage.classList.remove('active', 'zoomed', 'flying-to-tomo3');

    const droppedPhotoBackdrop = document.getElementById('droppedPhotoBackdrop');
    if (droppedPhotoBackdrop) droppedPhotoBackdrop.classList.remove('active');
  }
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (desk) {
    desk.addEventListener('click', (e) => {
      // Si la fotografía tomada está en la mesa pendiente de guardar, bloquear todo clic fuera
      ...
      if (e.target.closest('#tomo1BookWrapper') ||
          ...
          e.target.closest('#droppedPhotoBackdrop') ||
