...
window.Tomo3 = (function() {
  ...
  function setRemotePhotos(photos) {
    if (!Array.isArray(photos)) return;

    // Fusionar fotos de la nube con las locales sin duplicados
    const photoMap = new Map();
    photos.forEach(p => {
      photoMap.set(String(p.id), p);
    });
    savedPhotos.forEach(p => {
      if (!photoMap.has(String(p.id))) {
        photoMap.set(String(p.id), p);
      }
    });
  ...
  function addPhoto(photoItem) {
    const key = String(photoItem.id);
    const exists = savedPhotos.some(p => String(p.id) === key);
    if (!exists) {
    ...
    // Guardar en la nube en Firebase Firestore para que lo vean todos los dispositivos
    if (window.FirebaseSync && window.FirebaseSync.saveMemory) {
      window.FirebaseSync.saveMemory(photoItem);
    }
