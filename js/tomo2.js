...
window.Tomo2 = (function() {
  ...
  // Catálogo base de combinaciones de personas (14 combinaciones con sus variantes exactas)
  const BASE_CUTOUT_DEFS = [
    // 1 persona
    { base: '1J', count: 1, chars: ['J'], name: 'Juanca', variants: ['_00', '_01', '_02', '_03', '_04', '_05'] },
    { base: '1M', count: 1, chars: ['M'], name: 'Mamá', variants: ['_00', '_01', '_02', '_03', '_04', '_05', '_06'] },
    { base: '1P', count: 1, chars: ['P'], name: 'Papá', variants: ['_00', '_01', '_02', '_03', '_04'] },
    { base: '1Y', count: 1, chars: ['Y'], name: 'Yo', variants: ['_00', '_01', '_03', '_04', '_05', '_06', '_07', '_08'] },
    // 2 personas
    { base: '2MJ', count: 2, chars: ['M', 'J'], name: 'Mamá y Juanca', variants: ['_00', '_01', '_02', '_03'] },
    { base: '2MP', count: 2, chars: ['M', 'P'], name: 'Mamá y Papá', variants: ['_00', '_01', '_02', '_03'] },
    { base: '2MY', count: 2, chars: ['M', 'Y'], name: 'Mamá y Yo', variants: ['_00', '_01', '_02', '_03', '_04', '_05'] },
    { base: '2PJ', count: 2, chars: ['P', 'J'], name: 'Papá y Juanca', variants: ['_00', '_01', '_02', '_03'] },
    { base: '2PY', count: 2, chars: ['P', 'Y'], name: 'Papá y Yo', variants: ['_00', '_01', '_02', '_03', '_04'] },
    { base: '2YJ', count: 2, chars: ['Y', 'J'], name: 'Yo y Juanca', variants: ['_00', '_01', '_02', '_03', '_04', '_05', '_06', '_07', '_08', '_09', '_10', '_11', '_12', '_13', '_14'] },
    // 3 personas
    { base: '3MPJ', count: 3, chars: ['M', 'P', 'J'], name: 'Mamá, Papá y Juanca', variants: ['_00', '_01', '_02'] },
    { base: '3MPY', count: 3, chars: ['M', 'P', 'Y'], name: 'Mamá, Papá y Yo', variants: ['_00', '_01', '_02'] },
    { base: '3MYJ', count: 3, chars: ['M', 'Y', 'J'], name: 'Mamá, Yo y Juanca', variants: ['_00', '_01', '_02', '_03'] },
    { base: '3PYJ', count: 3, chars: ['P', 'Y', 'J'], name: 'Papá, Yo y Juanca', variants: ['_00', '_01', '_02', '_03', '_04', '_05', '_06'] }
  ];

  // Catálogo completo de 81 recortes fijados permanentemente
  const PERSON_CUTOUTS = [];
  BASE_CUTOUT_DEFS.forEach(def => {
    def.variants.forEach(variant => {
      const id = `${def.base}${variant}`;
      const fallbackId = `${def.base}${variant.replace('_', '')}`;
      PERSON_CUTOUTS.push({
        id: id,
        base: def.base,
        variant: variant,
        ...
        fallbackSrc: `FOTOS/RECORTES/${fallbackId}.png`
      });
    });
  });
  ...
  // Cálculo dinámico de pliegos: Pliego 0 (portadilla), Pliegos 1..N (contenido), Pliego N+1 (cortesía en blanco)
  function getTotalContentSpreads() {
    return Math.ceil(PERSON_CUTOUTS.length / 8);
  }

  function getMaxSpreadIndex() {
    return getTotalContentSpreads() + 1;
  }
  ...
  // Distribuye los recortes en el paisaje con solapamiento orgánico de aprox. 40px (~10.4%)
  // con jerarquía de profundidad: 3 personas adelante, 2 en medio y 1 detrás
  function getCutoutLayout(selectedIds, alignment) {
    ...
    const items = selectedIds.map(id => {
      return PERSON_CUTOUTS.find(c => c.id === id) || { id, name: id, count: 1 };
    });

    // Dimensiones según cantidad de recortes:
    let baseWidth = 28;
    let baseHeight = 47;
    let overlap = 10.4; // ~40px sobre el ancho de la postal de 386px
    ...
    const step = baseWidth - overlap;
    const totalGroupWidth = baseWidth + (N - 1) * step;

    // Ubicación clara según el tercio elegido aleatoriamente ('left', 'center', 'right'):
    let startX;
    if (alignMode === 'left') {
      startX = Math.max(6, Math.min(10, (100 - totalGroupWidth) * 0.16));
    } else if (alignMode === 'right') {
      startX = Math.max(6, 100 - totalGroupWidth - Math.max(6, Math.min(10, (100 - totalGroupWidth) * 0.16)));
    } else {
      startX = (100 - totalGroupWidth) / 2;
    }

    return items.map((item, index) => {
      const posLeft = startX + index * step;

      // Jerarquía de capas (profundidad):
      // 3 personas adelante (zIndex 30+), 2 personas al medio (zIndex 20+), 1 persona detrás (zIndex 10+)
      const layerDepth = ((item.count || 1) * 10) + index;

      return {
        id: item.id || selectedIds[index],
        item,
        left: `${posLeft.toFixed(1)}%`,
        bottom: '0%',
        width: `${baseWidth}%`,
        height: `${baseHeight}%`,
        zIndex: layerDepth
